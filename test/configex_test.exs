defmodule ConfigexTest do
  ...
  describe "get_env/3" do
    test "raise is configuration is missing" do
      assert Configex.get_env(:configex, :missing_conf) ==
               {:error, "Missing configuration: 'config :configex, missing_conf: <value>'"}
    end

    test "gets harcoded_nil" do
      assert Configex.get_env(:configex, :harcoded_nil) == {:ok, nil}
    end

    test "gets harcoded_value" do
      assert Configex.get_env(:configex, :harcoded_value) == {:ok, "Some harcoded value"}
    end

    test "raise is ENV var is missing" do
      assert Configex.get_env(:configex, :system_missing) ==
               {:error, "Missing ENV variable: 'MISSING_ENV'"}
    end

    test "gets system_env" do
      assert Configex.get_env(:configex, :system_env) == {:ok, "value 1"}
    end

    test "gets system_default" do
      assert Configex.get_env(:configex, :system_default) == {:ok, "some system default"}
    end
  end

  describe "get_env/3 with default" do
    test "gets default for missing_conf" do
      assert Configex.get_env(:configex, :missing_conf, "my-default") == {:ok, "my-default"}
    end

    test "gets harcoded_nil" do
      assert Configex.get_env(:configex, :harcoded_nil, "my-default") == {:ok, nil}
    end

    test "gets harcoded_value" do
      assert Configex.get_env(:configex, :harcoded_value, "my-default") ==
               {:ok, "Some harcoded value"}
    end

    test "gets default for system_missing" do
      assert Configex.get_env(:configex, :system_missing, "my-default") == {:ok, "my-default"}
    end

    test "gets system_env" do
      assert Configex.get_env(:configex, :system_env, "my-default") == {:ok, "value 1"}
    end

    test "gets system_default" do
      assert Configex.get_env(:configex, :system_default, "my-default") ==
